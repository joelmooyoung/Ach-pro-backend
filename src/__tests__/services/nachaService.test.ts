...
import { ACHTransaction, TransactionStatus } from '../../types';

describe('NACHAService - Compliance Validation', () => {
  let nachaService: NACHAService;
  ...
  beforeEach(() => {
    encryptionService = new EncryptionService('test-key-for-nacha-testing-12345678');
    nachaService = new NACHAService(mockConfig, encryptionService);
  });
  ...
  describe('Error Handling', () => {
    test('should throw error when encryption service required but not available', () => {
      const serviceWithoutEncryption = new NACHAService(mockConfig);
    ...
    test('should handle malformed configuration gracefully', () => {
      ...
      const serviceWithBadConfig = new NACHAService(badConfig, encryptionService);
